...
The <TypeLink type="ProcessMining" /> component features events that notify users when an item is either focused,
...
<Playground
    code={`
      function App() {
        ...
        (selectedItems: (ProcessStep | null)[]) =>
          console.log(
            selectedItems.length ? \`Selected: \${selectedItems[0].label}\` : 'Nothing selected'
          ),
        []
        )
        ...
        const onItemHover = useCallback(
          (item: ProcessStep | null) =>
            item && console.log(\`Hovered: \${item.label}\`),
          []
        )

        const onItemFocus = useCallback(
          (item: ProcessStep | null) =>
            item && console.log(\`Focused: \${item.label}\`),
          []
        )
